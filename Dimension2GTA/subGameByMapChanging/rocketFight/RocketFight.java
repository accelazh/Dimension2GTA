package subGameByMapChanging.rocketFight;
...
public class RocketFight extends SubGameByMapChanging
{
	private int AIAim=1;
	...
	public RocketFight(Player player,Human opponent,MainGameWindow master)
	{
	...
	public void editWeaponList()
	{
		...
		getPlayer().addWeapon(new RocketLauncher(100));
		getOpponent().addWeapon(new RocketLauncher(100));
	    
		Weapon[] pWeaponList=getPlayer().getWeaponList();
		Weapon[] oWeaponList=getOpponent().getWeaponList();
	
	    for(int i=0;i<pWeaponList.length;i++)
	    {
	    	if(pWeaponList[i] instanceof RocketLauncher)
	    	{
	    		getPlayer().setCurrentWeapon(i);
	    	    break;
	    	}
	    }
	    
	    for(int i=0;i<oWeaponList.length;i++)
	    {
	    	if(oWeaponList[i] instanceof RocketLauncher)
	    	{
	    		getOpponent().setCurrentWeapon(i);
	    	    break;
	    	}
	    }
	}
	...
	{
		getPlayer().setVest(new Vest(0.1,300));
		getOpponent().setVest(new Vest(0.1,300));
	}
