...
public class Spark2 implements Constants
{
	//for debug
	...
	public Spark2(MyPoint hitPoint,JPanel wallPanel)// hitPoint是JAVA坐标系的点
	{
	    ...
	    {
	    	double Rx=Math.cos(scatterArc);
            double Ry=Math.sin(scatterArc)*Math.cos((i+0.3)*turnArc);
            double Rz=Math.sin(scatterArc)*Math.sin((i+0.3)*turnArc);
	    ...
	    {
	    	double Rx=Math.cos(scatterArc);
            double Ry=Math.sin(scatterArc)*Math.cos((i+0.6)*turnArc);
            double Rz=Math.sin(scatterArc)*Math.sin((i+0.6)*turnArc);
